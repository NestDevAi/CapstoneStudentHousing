...
export const LandlordHeader = ({ activeTab, setActiveTab, user, onMenuClick }: HeaderProps) => {
  const [unreadCount, setUnreadCount] = useState(0);
  const [notifications, setNotifications] = useState<any[]>([]);
  const [isNotificationsOpen, setIsNotificationsOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const initials = user?.email?.split('@')[0].substring(0, 2).toUpperCase() || 'MS';
  const name = user?.full_name || user?.email?.split('@')[0] || 'Maria Santos';
    ...
    <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-10 sticky top-0 z-30">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-slate-400 overflow-hidden">
          <span className="text-sm font-medium hidden sm:block whitespace-nowrap">Landlord Portal</span>
        ...
        <div className="flex items-center gap-3 pl-6 border-l border-slate-100">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-[10px] text-slate-400 font-medium">
              {user?.is_verified ? 'Verified Property Owner' : 'Pending Verification'}
            </p>
          </div>
          <button 
            onClick={() => setActiveTab?.('profile')}
            className="w-10 h-10 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm hover:scale-105 transition-all shadow-lg shadow-emerald-500/20"
          >
            {initials}
          </button>
        </div>
